...
export async function sendMessage(instanceName, number, text, apiUrl = EVOLUTION_API_URL, apiKey = EVOLUTION_API_KEY) {
    // 1. Force @s.whatsapp.net suffix for SENDING
    // Even if we receive from @lid, we MUST send to @s.whatsapp.net
    const cleanDigits = number.replace(/\D/g, ''); 
    const finalJid = `${cleanDigits}@s.whatsapp.net`;
    
    // 2. Clean Text: Remove complex emojis and Normalize Unicode (Smart Quotes, etc)
    // This fixes "Bad Request" 400 errors caused by GPT returning non-standard characters
    let cleanText = text
