...
export async function sendMessage(instanceName, number, text, apiUrl = EVOLUTION_API_URL, apiKey = EVOLUTION_API_KEY) {
  try {
    // Evolution API v2: Requer `number` com JID (@s.whatsapp.net) e `text` no corpo (às vezes dentro de options, às vezes raiz)
    // A documentação mais recente sugere:
    // {
    ...
    // Garantir que temos apenas números e adicionar sufixo se faltar
    const cleanDigits = number.replace(/\D/g, '');
    const jid = cleanDigits.includes('@') ? cleanDigits : `${cleanDigits}@s.whatsapp.net`;
    ...
    // O endpoint v2 correto é frequentemente apenas /message/sendText/{instance}
    // Mas vamos manter compatível. Se falhar, o catch deve capturar sem derrubar o app.
