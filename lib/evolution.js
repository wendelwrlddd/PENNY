...
export async function sendMessage(instanceName, number, text, apiUrl = EVOLUTION_API_URL, apiKey = EVOLUTION_API_KEY) {
  try {
    // A Evolution API geralmente prefere o número PURO ou com @s.whatsapp.net dependendo da versão
    // O erro 400 sugere que o formato enviado está incorreto para a versão atual.
    // Vamos tentar enviar SOMENTE os dígitos se o JID falhar, ou garantir o JID correto.
    
    // Atualização: Versões recentes da Evolution (v2) preferem o remoteJid completo,
    // mas se o input já vier com @s.whatsapp.net, mantemos. Se não, adicionamos.
    // Porém, o LOG mostrou erro 400 com o sufixo. Vamos TENTAR enviar sem o sufixo para testar.
    
    // Tentativa 1: Enviar número LIMPO (apenas dígitos)
