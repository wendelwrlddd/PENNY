 ...
 * @param {Object} userData - Existing user profile data (income, payday)
 * @returns {Promise<Object>} - Structured transaction object
 */
export async function extractFinancialData(messageText, isBrazil = false, userData = {}) {
  try {
...
KNOWN USER PROFILE:
- monthlyIncome: ${userData.monthlyIncome || 'unknown'}
- payDay: ${userData.payDay || 'unknown'}
...
- RESET: To clear user profile and start over (e.g. "reset profile", "resetar meu perfil", "apagar meus dados").
...
4. Contextual Nuance: 
   - If the message is JUST a number (e.g., "1500", "5"):
     - If monthlyIncome is "unknown", it is a PROFILE_UPDATE (income).
     - If monthlyIncome is KNOWN and payDay is KNOWN, it is a SYNC (current balance).
     - If monthlyIncome is KNOWN but payDay is "unknown", it is likely a PROFILE_UPDATE (payDay).
...
User: "qual é o seu saldo atual? -> 1000"
JSON: {"intent": "SYNC", "amount": 1000, "type": "income"}
