...
export async function extractFinancialData(messageText, userData = {}, isBrazil = false, currentObjective = 'ACTIVE') {
  ...
  - RULES: If user says anything else (or just "hi"), YOU MUST REPEAT THE QUESTION.
  - MANDATORY QUESTION: "Hi! I’ll quickly set things up. How do you receive your income? 1️⃣ Hourly, 2️⃣ Weekly, 3️⃣ Fortnightly, 4️⃣ Monthly / Contract"
  - RESPONSE: Use the question above if no type is found. Confirm if found.
  ` : ''}

  ${currentObjective === 'ASK_HOURLY_RATE' ? `
  - TASK: Extract NUMBER for hourly rate.
  - RULES: Ignore expenses. If user doesn't give a number, politely ask: "What is your hourly rate?"
  - RESPONSE: If number found, confirm and ask: "And how many hours per week do you usually work?"
  ` : ''}

  ${currentObjective === 'ASK_WEEKLY_HOURS' ? `
  - TASK: Extract NUMBER for weekly hours.
  - RULES: Ignore expenses. If user doesn't give a number, politely ask: "How many hours per week?"
  - RESPONSE: If number found, confirm and ask: "Perfect. Last step: How much money do you have in your account right now?"
  ` : ''}
  ...
  - RESPONSE: If found, ask: "Perfect. Last step: How much money do you have in your account right now?"
  ` : ''}
  ...
  - RESPONSE: Confirm expense and show current balance: £${userData.currentBalance}. Include dashboard link.
  ` : ''}
