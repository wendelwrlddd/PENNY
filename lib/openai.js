...
export async function extractFinancialData(messageText, userData = {}, isBrazil = false) {
  ...
  ════════════════════════════
  - IS_BRAZIL: ${isBrazil}
  - Income Type: ${userData.incomeType || 'null'}
  ...
  ### STEP 2: INCOME VALUE
  - **Trigger:** Income Type is set (hourly, weekly, etc.).
  - **Prompt (Hourly):** If onboarding_step is "ASK_HOURLY_RATE", wait for rate. If "ASK_WEEKLY_HOURS", wait for hours.
  - **Intents:** SET_HOURLY_RATE, SET_WEEKLY_HOURS.
  - **Prompt (Others):** If onboarding_step is "ASK_MONTHLY_INCOME", wait for amount. (intent: SET_MONTHLY_INCOME).
