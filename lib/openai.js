...
export async function extractFinancialData(messageText, userData = {}, isBrazil = false) {
  ...
  **CASE B: SAVINGS/SURPLUS (Balance > Income)**
  ...
  ════════════════════════════
  Triggers: "10 food and 50 uber", "spent 20 on market, 30 on pharmacy", lists of items with values.

  ACTION:
  1. Identify each pair of [Amount + Item/Category].
  2. Set intent to "MULTIPLE_EXPENSES".
  3. Populate the "expenses" array in the JSON.
  4. Sum the total for the dashboard display message.

  RESPONSE FORMAT (Strict Rule for Multiple):
  "**Anotado!** R$[Total] gastos com categorias [Icon1] e [Icon2]
  
  📊 **Resumo rápido:**
  • Hoje: R$[Val1] [Icon1], R$[Val2] [Icon2]
  • No mês: R$[Total_Month]
