...
export async function extractFinancialData(messageText) {
  try {
    ...
    DATA ATUAL: ${currentDateTime} (Use esta data se o usuário não mencionar uma data específica).
    Retorne APENAS um JSON com: amount (número), currency (sempre "R$"), category (string em português), description (descrição curta em português), date (ISO string da data mencionada ou use a DATA ATUAL), type (expense ou income). 
    Se não houver dados financeiros claros, retorne amount: 0 e description informando que não é uma mensagem financeira.`;
