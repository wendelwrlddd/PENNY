    ...
    for (const modelName of modelsToTry) {
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          const prompt = `Extraia dados financeiros desta mensagem: "${messageText}". 
                          Data de hoje: ${currentDateTime}.
                          
                          REGRAS:
                          1. Se for uma despesa, type="expense". Se for ganho/recebimento, type="income".
                          2. Identifique a categoria (ex: Alimentação, Transporte, Lazer, etc).
                          3. amount deve ser um número (float).
                          4. Responda ESTRITAMENTE um objeto JSON válido.
                          
                          FORMATO:
                          {"amount": number, "currency": "R$", "category": string, "description": string, "date": string, "type": "expense" | "income"}`;
