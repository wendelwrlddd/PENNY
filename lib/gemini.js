      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          const prompt = `Extract financial data from this message: "${messageText}". 
                          ...
                          2. Identify the category in ${language} (e.g., Food, Transport, Leisure, Shopping, etc.).
                          3. amount must be a number (float).
                          4. Respond STRICTLY with a valid JSON object.
                          
                          FORMAT:
                          {"amount": number, "currency": "${currency}", "category": string, "description": string, "date": string, "type": "expense" | "income"}`;
