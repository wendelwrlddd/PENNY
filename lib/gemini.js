...
export async function extractFinancialData(messageText, isBrazil = false) {
  try {
    ...
    // --- Optimized rotation for Speed (Focus on Flash) ---
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          const prompt = `Extract financial data or user profile info from this message: "${messageText}". 
                          ...
                          INTENTS:
                          - RECORD: For a new specific expense or income.
                          - PROFILE_UPDATE: For monthlyIncome (how much they earn), isSalaried, and payDay.
                          - REMOVE: To delete a mistake.
                          - SYNC: When user reports their CURRENT TOTAL balance (e.g., "tenho só 181", "balance is 200", "total in bank 500"). Use this when they talk about what REMAINS in their account.
                          - UNCERTAIN: If the message is confusing or a type/suggestion is needed.
                          ...
                          3. If intent is SYNC:
                             - amount: the CURRENT balance reported (the number after "tenho só", "left", etc.).
                          4. If intent is UNCERTAIN:
                             - Provide a "suggestion" field in English of what you think the user meant.
                          5. Respond STRICTLY with valid JSON.
                          
                          FORMAT:
                          {
                            "intent": "RECORD" | "PROFILE_UPDATE" | "REMOVE" | "SYNC" | "UNCERTAIN",
                            "amount": number,
                            "currency": "${currency}",
                            "category": string,
                            "description": string,
