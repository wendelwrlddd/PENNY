...
export async function connectWhatsApp(onMessage, authFolder = './auth_info_baileys') {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // STOP THE LOOP: Do not automatically reconnect on 'undefined' (Zombie) or 'loggedOut'
      if (statusCode === DisconnectReason.loggedOut) {
        console.log('🚫 Logged out. Session ended. Waiting for manual restart/QR.');
        isConnected = false;
        return; 
      }

      if (!statusCode) {
         console.log('⚠️ Zombie connection closed (undefined error). NOT reconnecting automatically to avoid ban.');
         isConnected = false;
         return;
      }

      // Optional: Only reconnect on specific recoverable errors IF strictly necessary, 
      // but the user asked to STOP auto-reconnect. 
      // We will allow reconnect ONLY for "Restart Required" which is safe.
      if (statusCode === DisconnectReason.restartRequired) {
         console.log('🔄 Restart required. Reconnecting...');
         setTimeout(() => connectWhatsApp(onMessage, authFolder), 1000);
      } else {
